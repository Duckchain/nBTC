...
export default function Home() {
  return (
    <Layout>
      <section className="flex h-screen px-[2.5%] items-center">
        <div className="w-[60%]">
          <h1 className=" font-bold  text-transparent bg-clip-text bg-gradient-to-b from-[#5C1D70] to-[#2F073C] ">
            ...
            <br />{" "}
            <span className="text-[64px] leading-[84.54px]">Currency</span>
          </h1>
        ...
        <aside>
          <Image src={imgref.polygonshape} alt="nBTC logo" />
        </aside>
      </section>
      <div className=" mt-20 flex flex-col mb-[200px] gap-[200px] px-[2.5%]">
        {HalfSubArr.map((content, index) => (
          <section className={`${index % 2 === 0 ? "flex justify-end" : ""}`}>
            <HalfSub
              key={index}
      ...
      <div className=" bg-[#611F76] py-[84px] flex flex-col items-center text-white">
        <h1 className=" font-bold text-[50px] leading-[57.45px] flex flex-col items-center gap-[11px]">
        ...
        {BenefitBoysArr.map((content, index) => (
          <div
            ...
            } mt-[54px] flex flex-col gap-[23px] items-center text-center max-w-[862px]`}
          >
